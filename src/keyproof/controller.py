...
def acceptance(previous: EvaluationReport, candidate: EvaluationReport) -> tuple[bool, str]:
    ...
    return (
        True,
        f"Accepted: failing development gates reduced from {old_failures} to {new_failures}, with no regressions.",
    )
    ...
    return [
        {
            ...
            "patch": item.patch.model_dump() if item.patch else None,
...
async def run_repair(
    ...
    *,
    ...
    def event(role: str, kind: str, summary: str, data: dict | None = None) -> None:
        nonlocal sink
        item = RunEvent(
            sequence=len(record.events) + 1, role=role, kind=kind, summary=summary, data=data or {}
        )
    ...
    try:
        ...
        event(
            "controller",
            "started",
            f"Starting {config.mode} repair with independent keyboard gates.",
        )
        event(
            "provider", "identity", "Real model provider and aggregate run limits.", client.identity
        )
        ...
        record.initial_report = await evaluate_source(
            original,
            phase="development",
            artifact_dir=root / "initial" if root else None,
        )
        record.final_report = record.initial_report
        event(
            "evaluator",
            "development",
            "Original-source development evaluation completed.",
            {"report": record.initial_report.model_dump()},
        )
        if record.initial_report.errors:
            raise RuntimeError(
                "Original-source development evaluation failed: "
                + "; ".join(record.initial_report.errors)
            )
        ...
        for number in range(1, config.max_iterations + 1):
            ...
            event(
                "controller",
                "iteration",
                f"Starting repair attempt {number}.",
                {"iteration": number},
            )
            async with TaskBrowser(
                record.final_source, artifact_dir=root / f"audit-{number}" if root else None
            ) as browser:
                if config.mode == "team":
                    event(
                        "auditor",
                        "started",
                        "Auditor selecting keyboard actions from the live page.",
                    )
                    item.audit = await audit_task(client, browser, record.final_report, event)
                    event(
                        "engineer",
                        "started",
                        "Repair engineer reviewing source and independent auditor evidence.",
                    )
                    item.patch = await engineer_patch(
                        client, record.final_source, record.final_report, item.audit, history
                    )
                else:
                    event(
                        "single",
                        "started",
                        "Single agent exploring and repairing with the same keyboard tools.",
                    )
            ...
            event(
                "provider", "usage", "Observed cumulative model usage.", client.usage.model_dump()
            )
            event(
                "engineer" if config.mode == "team" else "single",
                "patch",
                item.patch.summary,
            ...
            try:
                ...
                _write_artifact(
                    root, f"iteration-{number}/attempt.json", item.model_dump_json(indent=2)
                )
                continue
            item.candidate_source = candidate
            item.diff = "".join(
                difflib.unified_diff(
                    record.final_source.splitlines(keepends=True),
                    candidate.splitlines(keepends=True),
                    fromfile="accepted/behavior.js",
                    tofile=f"candidate-{number}/behavior.js",
                )
            )
            ...
            item.report = await evaluate_source(
                candidate,
                phase="development",
                artifact_dir=root / f"iteration-{number}" if root else None,
            )
        ...
        record.status = (
            "completed"
            if record.final_report and record.final_report.passed
        ...
        if record.status == "budget_exhausted":
            event(
                "controller",
                "budget",
                "Iteration or model budget ended before all development gates passed.",
            )
    ...
    if task is not None and task.cancelling() and not cancelled:
        cancelled = True
        record.status = "failed"
        record.errors.append("Cancellation requested; teardown errors cannot restart work.")
    ...
    try:
        if record.final_report is not None and not cancelled:
            event(
                "controller",
                "frozen",
                "Final candidate frozen; no further model calls.",
            ...
            if evaluate_holdout:
                record.holdout_report = await evaluate_source(
                    record.final_source,
                    phase="holdout",
                    artifact_dir=root / "holdout" if root else None,
                )
                event(
                    "evaluator",
                    "holdout",
                    "Independent frozen-candidate holdout evaluation completed.",
        ...
        record.errors.append(
            f"Final evaluation/artifact failure: {type(exc).__name__}: {str(exc)[:1500]}"
        )
    ...
    event(
        "provider",
        "usage",
        "Final observed usage; null token counts mean unknown, not zero.",
        client.usage.model_dump(),
    )
    if record.errors:
        record.status = "failed"
    event(
        ...
        f"Run {record.status}.",
        {
            ...
            "development_passed": record.final_report.passed if record.final_report else None,
